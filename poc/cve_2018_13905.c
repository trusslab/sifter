...
void *thread_func(void *vargp) {
    int kgsl_fd = *(int *)vargp;
    //while (1) {
    for (int i = 0; i < ITER; i++) {
        int ret;
        //printf("1 c %d\n", kgsl_syncsource_create(kgsl_fd, 1));
        ret = kgsl_syncsource_create(kgsl_fd, 1);
        printf("1 c %d\n", ret);
        ret = kgsl_syncsource_destroy(kgsl_fd, 1);
        printf("1 d %d\n", ret);
    }
    return 0;
}
...
int main() {
    ...
    for (int i = 0; i < THREAD_NUM; i++) {
        counter[i] = 0;
        arg.fd = kgsl_fd;
        arg.counter = &counter[i];
        pthread_create(&tid, NULL, thread_func_1, (void *)&arg);
    }
