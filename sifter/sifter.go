...
func (sifter *Sifter) GenerateMapSection() {
	...
	if sifter.mode == FilterMode {
		if a := sifter.GetAnalysis("pattern analysis"); a != nil {
			...
			fmt.Fprintf(s, "DEFINE_BPF_MAP(syscall_seqs_curr, HASH, int, uint64_t, 1);\n")
		}
...
func (sifter *Sifter) GenerateHelperSection() {
	...
	if sifter.mode == FilterMode {
		if a := sifter.GetAnalysis("pattern analysis"); a != nil {
			...
			fmt.Fprintf(s, "\n")
			fmt.Fprintf(s, "    i = 0;\n")
			fmt.Fprintf(s, "    uint64_t *seq_order = bpf_syscall_seq_order_lookup_elem(this_id);\n")
			fmt.Fprintf(s, "    uint64_t *seqs_curr = bpf_syscall_seqs_curr_lookup_elem(&i);\n")
			fmt.Fprintf(s, "    if (seq_order && seqs_curr) {\n")
			fmt.Fprintf(s, "            if ((*seq_order & *seqs_curr) != 0) {\n")
			fmt.Fprintf(s, "                goto error;\n")
			fmt.Fprintf(s, "            } else {\n")
			fmt.Fprintf(s, "                *seqs_curr |= (1 << *this_id);\n")
			fmt.Fprintf(s, "            }\n")
			fmt.Fprintf(s, "    } else {\n")
			fmt.Fprintf(s, "        goto error;\n")
			fmt.Fprintf(s, "    }\n")
