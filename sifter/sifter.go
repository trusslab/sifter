...
//		fmt.Fprintf(s, "}\n")
	...
	if sifter.mode == FilterMode {
		...
		fmt.Fprintf(s, "            id_key.tag[0] = ctx->args[1];\n")
		fmt.Fprintf(s, "        } else if (nr == 222) {\n")
		fmt.Fprintf(s, "            ret = trace_mmap(ctx);\n")
		fmt.Fprintf(s, "            id_key.tag[0] = ctx->args[2];\n")
		fmt.Fprintf(s, "            id_key.tag[1] = ctx->args[3];\n")
		fmt.Fprintf(s, "        }\n")
		fmt.Fprintf(s, "        bpf_syscall_id_curr_update_elem(&ip, &id_key, BPF_ANY);\n")
